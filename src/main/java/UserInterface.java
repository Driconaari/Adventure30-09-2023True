...
public class UserInterface {
    ...
    private void interpretCommand(String input) {
        ...
        if (input.equals("health") || input.equals("hp")) {
            displayHealthStatus();
    ...
    private void handleEatCommand(String input) {
        // Split the input to get the food item's name
        String[] parts = input.split(" ");
        if (parts.length < 2) {
            System.out.println("Usage: eat [food item]");
            return;
        }
...
// Check if the specified food item exists in the current room or inventory
        ...
        if (foodItem == null || !(foodItem instanceof Food)) {
...
// Increase the player's health
        ...
        player.setHealth(currentHealth + healthPoints);
